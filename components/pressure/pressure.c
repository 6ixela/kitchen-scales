...
/**
 * @brief Sauvegarde les coefficients de calibration en mémoire non-volatile
 * (NVS)
 *
...
void calibration_save(void)
{
    ...
    err =
        nvs_set_blob(handle, "offset_g", &CALIBRATION_OFFSET_G, sizeof(float));
    if (err != ESP_OK)
...
/**
 * @brief Charge les coefficients de calibration depuis la mémoire non-volatile
 * (NVS)
 *
...
void calibration_load(void)
{
    ...
    if (err != ESP_OK)
    {
        printf("Error opening NVS handle: %s (utilisation des valeurs par "
               "défaut)\n",
               esp_err_to_name(err));
        return;
    }
